...
class GaussianDiffusionModel(nn.Module, ABC):
    ...
    def p_sample_step(
        self,
        x,                       # (B, H, D)
        hard_conds,              # dict of hard conds (already batched)
        context=None,
        t: int = None,           # a single timestep, can be negative for noiseless tail
        sample_fn=ddpm_sample_fn,
        **sample_kwargs,         # e.g., guide, n_guide_steps, t_start_guide, noise_std_extra_schedule_fn
        ):
        if t is None:
            raise ValueError("p_sample_step requires an integer timestep t")
        device = self.betas.device
        bsz = x.shape[0]
        tvec = make_timesteps(bsz, t, device)
        x, _ = sample_fn(self, x, hard_conds, context, tvec, **sample_kwargs)
        x = apply_hard_conditioning(x, hard_conds)
        return x
...
def alternating_block_gibbs_sample(
    ...
    *,
    ...
    state_dim_a: int,
    state_dim_b: int,
    hard_conds_a: Dict[str, torch.Tensor],
    hard_conds_b: Dict[str, torch.Tensor],
    context_a: Optional[Dict[str, torch.Tensor]] = None,
    context_b: Optional[Dict[str, torch.Tensor]] = None,
    return_chain: bool = True,
    n_diffusion_steps_without_noise: int = 0,
    sample_kwargs_a: Optional[Dict[str, Any]] = None,
    sample_kwargs_b: Optional[Dict[str, Any]] = None,
    init_x_a: Optional[torch.Tensor] = None,
    init_x_b: Optional[torch.Tensor] = None,
):
    ...
    x_a = init_x_a if init_x_a is not None else torch.randn(B, horizon_a, state_dim_a, device=device)
    x_b = init_x_b if init_x_b is not None else torch.randn(B, horizon_b, state_dim_b, device=device)
    ...
    def set_other_if_supported(kw: Dict[str, Any], other: torch.Tensor) -> None:
        g = kw.get('guide', None)
