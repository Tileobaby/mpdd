...
def experiment(
    ...
    **kwargs
    ...
    """
    #for dual robot:
    hard_conds_a = dataset_a.get_hard_conditions(torch.vstack((start_state_pos_a, goal_state_pos_a)), normalize=True)
    hard_conds_b = dataset_b.get_hard_conditions(torch.vstack((start_state_pos_b, goal_state_pos_b)), normalize=True)

    hard_conds_a = {k: einops.repeat(v, 'd -> b d', b=n_samples) for k, v in hard_conds_a.items()}
    hard_conds_b = {k: einops.repeat(v, 'd -> b d', b=n_samples) for k, v in hard_conds_b.items()}

    # Optional contexts (normalize & repeat if you use context_model)
    context_a = None
    context_b = None
    ...
    with TimerCUDA() as timer_model_sampling:
        trajs_chain_a_norm, trajs_chain_b_norm = alternating_block_gibbs_sample(
            model_a=diffusion_model_a,            # or 'model' twice if same robot/model
            model_b=diffusion_model_b,
            horizon_a=dataset_a.n_support_points,
            horizon_b=dataset_b.n_support_points,
            state_dim_a=dataset_a.state_dim,
            state_dim_b=dataset_b.state_dim,
            hard_conds_a=hard_conds_a,
            hard_conds_b=hard_conds_b,
            context_a=context_a,
            context_b=context_b,
            return_chain=True,
            n_diffusion_steps_without_noise=n_diffusion_steps_without_noise,
    ...
    # Unnormalize back to joint space
    trajs_chain_a = dataset_a.unnormalize_trajectories(trajs_chain_a_norm)  # (S,B,H,D)
    trajs_chain_b = dataset_b.unnormalize_trajectories(trajs_chain_b_norm)
    ...
    if render:
        ...
        if isinstance(robot, RobotFanuc):
            # visualize in Isaac Gym
            # POSITION CONTROL
            # add initial positions for better visualization
            n_first_steps = 10
            ...
            motion_planning_isaac_env = FanucMotionPlanningIsaacGymEnv(
                env, robot, task,
                asset_root="../../deps/isaacgym/assets",
                fanuc_asset_file="urdf/fanuc/urdf/fanuc.urdf",  # <- put your actual URDF path
                ee_link_name="tool0",  # <- set to your URDF’s EE link name
                controller_type='position',
                num_envs=trajs_pos.shape[1],
                all_robots_in_one_env=True,
                color_robots=False,
